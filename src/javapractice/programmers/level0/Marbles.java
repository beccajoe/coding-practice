...
public class Marbles {
    public BigInteger solution(int balls, int share) {
        ...
        for (int i = 1; i <= balls; i++){
            ballFactorial = ballFactorial.multiply(BigInteger.valueOf(i));
        }
        BigInteger shareFactorial = BigInteger.ONE;
        for (int i = 1; i <= share; i++){
            shareFactorial = shareFactorial.multiply(BigInteger.valueOf(i));
        }
        BigInteger ballsMinusShareFactorial = BigInteger.ONE;
        for (int i = 1; i <= balls - share; i++){
            ballsMinusShareFactorial = ballsMinusShareFactorial.multiply(BigInteger.valueOf(i));
        }
        BigInteger answer = ballFactorial.divide(shareFactorial.multiply(ballsMinusShareFactorial));
        return answer;
    }
}
// 구슬을 나누는 경우의 수
